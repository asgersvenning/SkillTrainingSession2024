...
syn_gam_rsq <- round(summary(syn_gam)$r.sq, 3)
syn_bam_rsq <- round(summary(syn_bam)$r.sq, 3)
...
As you can see `bam` can be **much** faster tham `gam`, however does it come with a downside? The answer is most of the time 'no'. In this case the $r^2$ for the two models is almost identical: **`gam`= `r syn_gam_rsq`** & **`bam`= `r syn_bam_rsq`**. And if we look at their fits, they are also almost identical:
...
![](timing_plt.png) That is not a carte blanche to use `bam` in all situations, or to say that it doesn't have downsides. For example, `bam` is not as flexible as `gam` and can't handle some of the more complex models that `gam` can. It is also not always as robust as `gam` and can sometimes fail to converge, especially when the data is not well-behaved or when used with some of the more complex families (such as `betar`) or smooth terms.
